...
例えば広告のURLを `https://広告のドメイン.com` とすると通常のDNSサーバーが正しいIPアドレス(`123.456.789.xxx`)を返してくれるので、広告がそのまま表示されます。これを広告ブロックDNSサーバーでは通常のIPアドレスではなく `0.0.0.0（または127.0.0.1）`を返すように指定することによって、本来のサーバーにアクセスすることができなくなる＝ブロックできるようになります。  
